...
class SingleFrequency:
    ...
    def open(self):
        ...
        self.write(":UNIT:POW V")          # 纵轴刻度单位设置为V
        self.log("[频谱仪] 纵轴刻度单位已设置为V")
    ...
    def get_trace_xy(self):
        try:
            ...
            #self.log(f"[调试] 原始数据类型: {type(raw_data)}, 长度: {len(raw_data) if hasattr(raw_data, '__len__') else '无长度'}")
            # if raw_data:
            #     self.log(f"[调试] 前5个数据点: {raw_data[:5]}")
            #     self.log(f"[调试] 后5个数据点: {raw_data[-5:]}")
            
            # 将线性功率值（瓦）转换为dBm值
            # 公式：dBm = 10 * log10(P / 1e-3)
            y_linear = np.array(raw_data)
            #self.log(f"[调试] 线性功率最小值: {np.min(y_linear) if len(y_linear) > 0 else 'N/A'} W, 最大值: {np.max(y_linear) if len(y_linear) > 0 else 'N/A'} W")
            
            # 避免除以零或负数
            y_linear = np.maximum(y_linear, 1e-15)  # 设一个最小阈值
            y_dbm = 10 * np.log10(y_linear / 1e-3)
            
            #self.log(f"[调试] 转换后dBm数据最小值: {np.min(y_dbm) if len(y_dbm) > 0 else 'N/A'} dBm, 最大值: {np.max(y_dbm) if len(y_dbm) > 0 else 'N/A'} dBm")
